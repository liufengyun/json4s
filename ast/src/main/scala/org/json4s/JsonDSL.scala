...
trait Implicits {
  implicit def short2jvalue(x: Short): JValue = JInt(BigInt(x))
  implicit def byte2jvalue(x: Byte): JValue = JInt(BigInt(x))
  implicit def char2jvalue(x: Char): JValue = JInt(BigInt(x))
...
trait JsonDSL extends Implicits {
  ...
  implicit def seq2jvalue[A](s: Traversable[A])(implicit ev: A => JValue): JArray =
